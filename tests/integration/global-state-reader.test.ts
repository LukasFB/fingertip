...
test("global-state reader accepts ChatGPT state files larger than four MiB", async (context) => {
  ...
  await writeFile(statePath, JSON.stringify({
    "electron-saved-workspace-roots": ["/Users/test/Project"],
    "queued-follow-ups": {
      "00000000-0000-4000-8000-000000000001": [{ text: "x".repeat(4 * 1024 * 1024) }],
    },
  }));
  ...
  assert.deepEqual(metadata.savedRoots, ["/Users/test/Project"]);
  assert.equal(metadata.queuedFollowUpCounts.get(parseTaskId("00000000-0000-4000-8000-000000000001")), 1);
  assert.equal(JSON.stringify(metadata).includes("xxxxx"), false);
});
